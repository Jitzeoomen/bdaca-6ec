...
\small
\begin{alertblock}{JSON lines (ndJSON)}
There is also a dialect in which you write \emph{one JSON object per line instead of per file}. For this, you can use a for loop as in the one-string-per-line example, but convert each string with  \texttt{json.load\textbf{s}} or \texttt{json.dump\textbf{s}} to a dict.
\end{alertblock}
...
$\Rightarrow$ In the exercises belonging to today's class you can work with a simple API.
